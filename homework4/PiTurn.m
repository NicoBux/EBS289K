...
%Pi Turn
global RL N Rmin npoints
if row>=N+2 %top turning
    if current(1)-next(1)>0 
        % going right to left at top
        a = linspace(pi/2,0,npoints); %first turn
        x1 = Rmin*sin(a)+current(1)-Rmin;
        y1 = Rmin*cos(a)+RL;

        c = linspace(0,-pi/2,npoints); %second turn
        x3 = (Rmin*sin(c))+next(1)+Rmin;
        y3 = Rmin*cos(c)+RL;

        x2 = linspace(x1(end),x3(1),npoints); %straight path
        y2 = linspace(y1(end),y1(end),npoints);
    ...
    if current(1)-next(1)>0 %right to left
        a = linspace(pi/2,pi,npoints); %first turn
        x1 = Rmin*sin(a)+current(1)-Rmin;
        y1 = Rmin*cos(a);

        c = linspace(pi,3*pi/2,npoints); %second turn
        x3 = (Rmin*sin(c))+next(1)+Rmin;
        y3 = Rmin*cos(c);
        ...
        c = linspace(pi,pi/2,npoints); %second turn
        x3 = (Rmin*sin(c))+next(1)-Rmin;
        y3 = Rmin*cos(c);
